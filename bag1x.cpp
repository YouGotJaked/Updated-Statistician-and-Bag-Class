...
namespace coen70_lab2 {
    ...
    /* Member function that removes used contents of subend from the current bag
     */
    void bag::operator-=(const bag& subend) {
        for (size_type i = 0; i < subend.used; i++) {
            erase_one(subend.data[i]);
        }
    }
    ...
    /* Non-member function that returns a new bag with the contents of b1 minus
     * the contents shared by it with b2
     */
    bag operator-(const bag& b1, const bag& b2) {
        bag answer;
